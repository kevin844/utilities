...
"""
cipher.cipher_file('samples1.exe','cipher.aaa')
cipher.save_iv_key('key','iv')
...
cipher = CipherFiles()
cipher.cipher_file('dfirCoockbook.md','cipher.aaa')
cipher.save_iv_key('key','iv')

#cipher.load_iv_key('key','iv')
#cipher.decrypter('cipher.aaa','dcrypt_sample.exe',cipher.key,cipher.iv)
...
cipher.load_iv_key('key','iv')
cipher.decrypter('cipher.aaa','dcrypt.md',cipher.key,cipher.iv)
        ...
        with open(input_file, "rb") as in_file, open(output_file, "wb") as out_file:
            ...
            while True:
                ...
                if not block:
                    break
                alloc_block = (
                    padder.update(block) + padder.finalize()
                    if len(block) < 16
                    else padder.update(block)
                )
                out_file.write(file_cipher.update(alloc_block))
            out_file.write(file_cipher.finalize())
"""